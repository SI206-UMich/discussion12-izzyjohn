...
def add_employee(filename, cur, conn):
    ...
    for item in data:
        ...
        job_id = item['job_id']
        salary = item['salary']
        cur.execute('insert or ignore into employees (employee_id, first_name, last_name, job_id, hire_date, salary) values (?,?,?,?,?,?)', (emp_id, f_name, l_name, hire_date, job_id, salary))
    conn.commit()
    pass
